...
func harvest_state(p_starting_day: int, current_day: int) -> void:
	var days_passed: int = current_day - p_starting_day
	
	if days_passed >= days_until_harvest - 1:
		current_growth_state = DataTypes.GrowthStates.Harvesting
		print("Harvest stage reached! Spawning collectable...")
